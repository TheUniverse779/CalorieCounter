...
import com.caloriecounter.calorie.ui.main.adapter.ImageHomeAdapter
...
class HomeTabFragment : BaseFragment<FragmentHomeTabBinding?>() {
    ...
    private var imageAdapter: ImageHomeAdapter? = null
...
//        }
    ...
    private fun getData() {
        if (canLoadMore) {
            ...
            canLoadMore = false
            weatherViewModel.getImagesByCatId(null, sort, null, null,"45", offset)
            try {
                WeatherApplication.trackingEvent("Load_more_data_Home", "Offset", offset.toString())
            } catch (e: Exception) {
            }
        }
    }
    ...
    override fun setObserver() {
        weatherViewModel.dataResponseLiveData.observe(this, Observer {
            ...
            for (i in 0 until 5){
                ...
                list.add(it.items[7])
                list.add(it.items[8])
                promotionItem.images = list
                listImage.add(promotionItem)
                it.items.removeAt(0)
                it.items.removeAt(0)
                it.items.removeAt(0)
                it.items.removeAt(0)
